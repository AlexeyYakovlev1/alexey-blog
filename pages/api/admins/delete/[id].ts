...
const deleteHandler = authMiddleware(async (req: NextApiRequest, res: NextApiResponse) => {
    const { method, query } = req;

    if (method === "DELETE") {
        try {
            const data = [query.id];
            const queryForSearch = `SELECT * FROM person WHERE id = $1`;
            ...
            const queryForDelete = `DELETE FROM person WHERE id = $1`;
